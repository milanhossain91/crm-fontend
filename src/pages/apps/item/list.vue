...
const deleteService = async id =>{
  try {
    const res = await $api(`${baseUrl}/items/${id}`, {
      method: "DELETE",
    })
    ...
    <VCard>
      <VCardText class="d-flex flex-wrap py-4 gap-4">
        ...
        <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
          ...
          <VBtn
            ...
            @click="() => router.push({name:'apps-item'})"
          >
            Add New
          </VBtn>
        </div>
      ...
      >
        ...
        <template #item.status="{ item }">
          <span :class="`${item?.status == 0 ? 'text-secondary' : 'text-primary'} px-3 py-1 rounded-full cursor-pointer hover:bg-gray-200 transition-colors`">
