...
const fetchUsers = async () => {
  try {
    ...
    const res = await $api(`${baseUrl}/key-account-persons`, {
      method: "GET",
    })
...
const deleteService = async id =>{
  try {
    const res = await $api(`${baseUrl}/key-account-persons/${id}`, {
      method: "DELETE",
    })
      ...
      <VCardText class="d-flex flex-wrap py-4 gap-4">
        ...
        <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
          ...
          <VBtn
            ...
            @click="() => router.push({name:'apps-keyaccountpersons'})"
          >
            Add New
          </VBtn>
        </div>
      ...
      >
        ...
        <template #item.actions="{ item }">
          ...
          <IconBtn
            @click="
              $router.push({
                name: 'apps-keyaccountpersons-edit-id',
                params: { id: item.id },
                query: {
                  fname: item?.name,
                  slug: item?.slug,
