...
const deleteList = async slug =>{
  try {
    const res = await $api(`${baseUrl}/departments/${slug}`, {
      method: "DELETE",
    })
...
const handleSearch = () => {
  ...
  } else department.value = backup.value
}
</script>
      ...
      <VCardText class="d-flex flex-wrap py-4 gap-4">
        ...
        <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
          ...
          <VBtn
            ...
            @click=" $router.push({
              name: 'apps-employees-departments-create',
            })"
          >
            Add New
          </VBtn>
      ...
      >
        ...
        <template #item.actions="{ item }">
          ...
          <IconBtn
            @click="
              $router.push({
                name: 'apps-employees-departments-id',
                params: { id: item.id },
                query: {
                  fname: item?.name,
                  slug: item?.slug,
